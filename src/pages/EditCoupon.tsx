...
const EditCoupon = () => {
    ...
    const fetchCoupon = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          free_shipping: couponData.free_shipping || false,
          ...
          user_condition: {
            ...
            emails: couponData.user_condition?.emails || '',
            purchased: couponData.user_condition?.purchased || ''
          },
        ...
        if (couponData.end_date) {
          setEndDate(new Date(couponData.end_date));
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) {
      toast({
        title: 'Error',
        description: 'Coupon ID is missing',
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      ...
      await couponService.update(parseInt(id), couponData);
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input 
                  ...
                  className="mt-1"
                  value={formData.condition?.min_purchase_amount || ''}
                  onChange={(e) => {
                    const value = parseFloat(e.target.value) || 0;
                    setFormData(prev => ({
                      ...prev,
              ...
              <div>
                ...
                <Input 
                  ...
                  className="mt-1"
                  value={formData.condition?.min_purchase_qty || ''}
                  onChange={(e) => {
                    ...
                    {loadingGroups ? (
                      ...
                      groups.map((group) => (
                        <div key={group.id} className="flex items-center space-x-2">
                          <Checkbox 
                            id={`group-${group.id}`}
                            checked={formData.user_condition?.groups?.includes(group.id)}
